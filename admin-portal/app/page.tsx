...
export default function StandaloneAdminPage() {
  ...
  // Fetch admin data from connected backend
  const loadAdminData = async () => {
    ...
    try {
      ...
      const [usersRes, statsRes] = await Promise.all([
        fetch(`${apiHost}/api/admin/users`).then((r) => (r.ok ? r.json() : { users: [] })),
        fetch(`${apiHost}/api/admin/stats`).then((r) => (r.ok ? r.json() : null)),
      ]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#2f3031] pb-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-[#9c9c9d]">
            Connected to SiteCompiler Backend & Firestore. Port: 3002.
          </p>
        </div>
