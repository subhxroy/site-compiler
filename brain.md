...
28. **Phase 1: Deterministic Node Tagging, Site Model & In-Browser Live Editor (`lib/model/`)** —
   - `lib/model/node-tagger.ts`: FNV-1a 32-bit deterministic DOM tagger attaches `data-sc-id="sc_<hash>"` to text nodes (`h1..h6, p, span, a, li, blockquote, button`) and `<img>` tags on the cleaned Cheerio AST. It is strictly idempotent and skips internal elements (`#sitecompiler-critical`, scripts, styles).
   - `lib/model/extract-model.ts`: Extracts `SiteModel` JSON map with node metadata (`type: 'text' | 'image'`, `content`, `src`, `alt`, `tag`, `domPath`) saved to `output/html-export/site-model.json`.
   - `lib/model/apply-patch.ts` & `lib/model/patch-job.ts`: Atomic point-patch mutation engine that strictly modifies `.text(content)` and `.attr('src'|'alt', value)` on exact `[data-sc-id]` selectors without touching `.html()`, inner/outer structures, or CSS class names. Sanitizes image sources to local `./assets/` or base64 data URIs. Staged changes are validated via `validateHtmlOutput()` before atomic replacement and re-zipping via `createJobZip()`.
   - `app/api/job/[id]/model/route.ts` & `server/index.ts`: `GET /api/job/:id/model` and rate-limited (20/min) `POST /api/job/:id/model` for patch application.
   - `app/edit/[jobId]/page.tsx`: In-browser visual live editor embeds `/api/job/:id/preview`, injects iframe contenteditable bridge on `[data-sc-id]` nodes, tracks dirty patches, provides live save, instant re-zipping, and discard capabilities.
   - `lib/zip/build-zip.ts` explicitly excludes `.sc-tagged/` directory from user export ZIP archives.
